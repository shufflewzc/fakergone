...
const {shareCodeKeys, updateEnvs,pushCodes} = require('./utils/updateEnv.js')
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map((item) => {
                  if (item.taskType === 14) {
                    pushCodes(shareCodeKeys.ddgc, item.assistTaskDetailVo.taskToken, $.index)
                    console.log(
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (data.factoryList && data.productionList) {
                  ...
                  pushCodes(shareCodeKeys.jxgc, data.user.encryptPin, $.index)
      ...
      try {
        if (err) {
          ...
          if (
            ...
            pushCodes(shareCodeKeys.ddmc, $.petInfo.shareCode, $.index)
            console.log(
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if ($.taskList.filter(item => !!item && item['taskId']=== 3) && $.taskList.filter(item => !!item && item['taskId']=== 3).length) {
                const code = $.taskList.filter(item => !!item && item['taskId']=== 3)[0]['itemId']
                pushCodes(shareCodeKeys.jdzz, code, $.index)
                console.log(`【京东账号${$.index}（${$.UserName}）的京东赚赚好友互助码】${code}`);
              }
            }
          }
        } catch (e) {
...
async function getPlantBean() {
  ...
  async function jdPlantBean() {
    ...
    if ($.plantBeanIndexResult.code === "0") {
      ...
      pushCodes(shareCodeKeys.zddd, $.myPlantUuid, $.index)
...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
      pushCodes(shareCodeKeys.ddnc, $.farmInfo.farmUserPro.shareCode, $.index)
      console.log(
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success && data.data && data.data.userInviteCode) {
              pushCodes(shareCodeKeys.crazyJoy, data.data.userInviteCode, $.index)
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if (data.data.bizCode === 0) {
            ...
            pushCodes(shareCodeKeys.sgmh, invites[0]['assistTaskDetailVo']['taskToken'], $.index)
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if(data.code===0 && data.data.result){
              pushCodes(shareCodeKeys.qdlxj, data.data.result.inviteCode, $.index)
