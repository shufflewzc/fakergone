/*
修改自faker2原始jd_get_share_code.js
主要功能：为面板内提供的jd账号生产各个活动的互助码环境变量
前置条件：青龙面板->系统设置->应用设置 新增SetEnv应用。且给出【环境变量】权限
...
================Loon==============
[Script]
cron "0 1 * * *" script-path=jd_generate_share_code_env,tag=生成互助码环境变量
 */
const $ = new Env("生成互助码环境变量");
const {shareCodeKeys, updateEnv} = require('./utils/updateEnv.js')
const DDNC = [];
const ZDDD = [];
const JXGC = [];
const QDLXJ = [];
...
function pushCodes(arr, code, cookieIndex) {
    arr.push({
        code,
        cookieIndex
    })
...
!(async () => {
    ...
    .finally(async () => {
        await updateEnv(shareCodeKeys.ddnc, (DDNC));
        await updateEnv(shareCodeKeys.zddd, (ZDDD));
        await updateEnv(shareCodeKeys.ddmc, (DDMC));
        await updateEnv(shareCodeKeys.jxgc, (JXGC));
        await updateEnv(shareCodeKeys.sgmh, (SGMH));
        await updateEnv(shareCodeKeys.qdlxj, (QDLXJ));
        $.done();
    })
                        ...
                        if (safeGet(data)) {
                            ...
                            if (data["ret"] === 0) {
                                ...
                                if (data.factoryList && data.productionList) {
                                    ...
                                    pushCodes(JXGC, data.user.encryptPin, $.index)
            ...
            try {
                if (err) {
                    ...
                    if (
                        ...
                        pushCodes(DDMC, $.petInfo.shareCode, $.index)
                        console.log(
...
async function getPlantBean() {
    ...
    async function jdPlantBean() {
        ...
        if ($.plantBeanIndexResult.code === "0") {
            ...
            pushCodes(ZDDD, $.myPlantUuid, $.index)
...
async function getJDFruit() {
    ...
    async function jdFruit() {
        ...
        if ($.farmInfo.farmUserPro) {
            pushCodes(DDNC, $.farmInfo.farmUserPro.shareCode, $.index)
            console.log(
            ...
            $.post(url, async (err, resp, data) => {
                try {
                    ...
                    if (data.data.bizCode === 0) {
                        ...
                        pushCodes(SGMH, invites[0]['assistTaskDetailVo']['taskToken'], $.index)
                ...
                if (err) {
                    ...
                    if (safeGet(data)) {
                        ...
                        if (data.code === 0 && data.data.result) {
                            pushCodes(QDLXJ, data.data.result.inviteCode, $.index)
